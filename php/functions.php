...
function addPost($connect, $data) {
    ...
    mysqli_query($connect, "INSERT INTO `posts` (`id`, `title`, `body`, `phone`) VALUES (NULL, '$title', '$body', '$phone')");
...
function updatePost($connect, $id, $data) {
    ...
    $body = $data['body'];
    $phone = $data['phone'];
    mysqli_query($connect, "UPDATE `posts` SET `title` = `$title` WHERE `posts`.`id` = $id");
